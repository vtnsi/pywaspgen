...
class IQDatagen:
    ...
    def _get_iq(self, burst):
        ...
        if samples.size != 0:
            ...
            print('SNR est before: ', 10*np.log10(np.mean(np.abs(samples)**2)))
            samples = np.sqrt((10.0 ** (burst.metadata["snr"] / 10.0)) / 2.0) * samples
            print('SNR est after: ', 10*np.log10(np.mean(np.abs(samples)**2)))
    ...
    def gen_iqdata(self, burst_list, data_idx=-1):
        ...
        for burst in burst_list:
            ...
            print('SNR est: ', 10*np.log10(np.mean(np.abs(samples)**2)))
